...
# --- 1. Cấu hình Web Server Flask (Giữ cho Render không bị Sleep) ---
app = Flask('')
...
class PrisonerSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not prison_role:
            await interaction.response.send_message("❌ Không tìm thấy vai trò **Tù Nhân**! Vui lòng tạo vai trò này trong Server trước.", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.response.send_message("❌ Bot thiếu quyền (Hãy kéo Vai trò của Bot lên cao hơn người muốn bắt)!", ephemeral=True)
...
class MainPanelView(View):
    ...
    async def create_ticket_channel(self, interaction: discord.Interaction, prefix: str):
        # Báo cho Discord biết bot đang xử lý để tránh lỗi Timeout
        await interaction.response.defer(ephemeral=True)

        guild = interaction.guild
        member = interaction.user
        
        # Tìm hoặc tạo đúng danh mục PHIÊN TÒA XÉT XỬ
        category = discord.utils.get(guild.categories, name="PHIÊN TÒA XÉT XỬ")
        if not category:
            category = await guild.create_category("PHIÊN TÒA XÉT XỬ")
